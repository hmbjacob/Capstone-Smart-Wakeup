...
{ 
	...
	while(upload_confirmed && (millis() - boot_start < 30000 || code_uploading))
	{
		if(!code_uploading)
		{
			if((millis()%1000) < 500)
				NRF_GPIO->OUTCLR = 1<<pin_led_r;				
			else
				NRF_GPIO->OUTSET = 1<<pin_led_r;				
		}
//		check_uart_packet();
